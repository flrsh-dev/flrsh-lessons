...
In order to execute the generate code, we have to explicitly instruct it to do so. This is can be done by either calling the function `collect()` which will _collect_ the results into a tibble. Or, we can use the functions `as.data.frame()`, `as.data.table()` or `as_tibble()` to choose how we want the results stored.
